myApp.controller('loginController', ['$scope', '$location', 'AuthService', function ($scope, $location, AuthService) {
      ...
      $scope.login = function () {
            ...
            if ($scope.loginForm == undefined)
                  ...
                  $scope.error = true, $scope.errorMessage = "User already logged in you need to logout first";
            }
            else
      ...
      $scope.register = function () {
            ...
            if ($scope.registerForm == undefined)
                  $scope.error = true, $scope.registerErrorMessage = "Please don't leave any field empty!";
            else
                  AuthService.register($scope.registerForm.email, $scope.registerForm.password, $scope.registerForm.email)
                        .then(() => {
                              $location.path('/main');
                        }, () => {
                              $scope.error = true;
...
myApp.controller('mainController', ['$scope', function ($scope) {
      ...
      $scope.labels = ['Semester 1', 'Semester 2', 'Summer 1', 'Semester 3', 'Semester 4', 'Summer 2', 'Semester 5'];
